...
def generate_chzzk_timeline(input_script, chat_script="", actual_title="VOD제목", chzzk_url="", api_key="", chunk_index=0):
    ...
    try:
        ...
        print(f"💾 [통합 백업] 후처리 전 원본 JSON 누적 완료 -> {combined_json_path}")
        
    except Exception as save_err:
        print(f"⚠️ 후처리 전 JSON 통합 저장 중 오류 발생 (진행은 유지됨): {save_err}")
...
def Final_Processing(timeline_text: str, api_key: str, db_filename="chzzk_streamers.txt") -> str:
    ...
    try:
        ...
        print(f"⚠️ [Gemini 검수 예외] 1차 전제본으로 대체합니다: {e}")
